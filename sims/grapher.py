...
class Database(object):
    ...
    def add_column(self, table, colname, type="TEXT"):
        try:
            self.write(Template('''ALTER TABLE "$table" ADD COLUMN "$colname" $type;''').substitute(table = table,
    ...
    def prepare_base(self):
        if not self.table_exists("experiments"):
            self.write('''CREATE TABLE experiments (exp_id INTEGER, PRIMARY KEY(exp_id ASC));''')

    def prepare_parameters(self, params):
        #learn vars and types from read parameters
        for arg, val in params.iteritems():
            self.add_column('experiments', arg, TypeHelper.getType(val))


    def prepare_results(self, results):
        for col, res in results.iteritems():
            self.add_column('experiments', col, TypeHelper.getType(res))
        # for var, treatmentlist in EXP_RESULTS.iteritems():
    ...
    def write_results(self, results):
        ...
        self.write(cmd%("','".join(results.keys()), xs), results.values())
